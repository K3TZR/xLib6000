...
public final class Cwx : NSObject, StaticModel {
  ...
  func replyHandler(_ command: String, seqNum: UInt, responseValue: String, reply: String) {
    ...
    guard components == 1 || components == 2 else {
      _log(Api.kName + ": \(command), Invalid Cwx reply", .warning, #function, #file, #line)
      return
    }
    ...
    guard charPos != nil else {
      _log(Api.kName + ": \(command), Invalid Cwx character position", .warning, #function, #file, #line)
      return
    }
    ...
    if components == 1 {
      ...
      guard block != nil else {
        ...
        _log(Api.kName + ": \(command), Invalid Cwx block", .warning, #function, #file, #line)
        return
      }
    ...
    for property in properties {
      ...
      if property.key.hasPrefix("macro") && property.key.lengthOfBytes(using: String.Encoding.ascii) > 5 {
        ...
        guard let token = Token(rawValue: property.key) else {
          ...
          _log(Api.kName + ": Unknown Cwx token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
          continue
        }
