...
public final class Cwx : NSObject, StaticModel {
  ...
  func replyHandler(_ command: String, seqNum: UInt, responseValue: String, reply: String) {
    ...
    guard components == 1 || components == 2 else {
      _log(Self.className() + " \(command), Invalid reply", .warning, #function, #file, #line)
      return
    }
    ...
    guard charPos != nil else {
      _log(Self.className() + " \(command), Invalid character position", .warning, #function, #file, #line)
      return
    }
    ...
    if components == 1 {
      ...
      guard block != nil else {
        ...
        _log(Self.className() + " \(command), Invalid block", .warning, #function, #file, #line)
        return
      }
