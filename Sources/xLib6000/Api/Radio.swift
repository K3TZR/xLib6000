...
//    case VOICE
  ...
  private func parseV3Connection(properties: KeyValuesArray, handle: Handle) {
    ...
    for property in properties.dropFirst(2) {
      ...
      guard let token = ClientTokenV3Connection(rawValue: property.key) else {
        ...
        _log(String(describing: Self.self) + ": Unknown Client Connection token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
  ...
  private func parseV3Disconnection(properties: KeyValuesArray, handle: Handle) {
    ...
    for property in properties.dropFirst(2) {
      ...
      guard let token = ClientTokenV3Disconnection(rawValue: property.key) else {
        // log it and ignore this Key
        _log(String(describing: Self.self) + ": Unknown Client Disconnection token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
      // Known keys, in alphabetical order
      switch token {
      ...
      if handle == _api.connectionHandle && (duplicateClientId || forced || wanValidationFailed) {
        _log(String(describing: Self.self) + ": Disconnected with: \(forced ? "Forced ": "")\(duplicateClientId ? "DuplicateClientId ": "")\(wanValidationFailed ? "wanValidationFailed": "")" , .warning, #function, #file, #line)
  ...
  private func parseMessage(_ commandSuffix: String) {
    ...
    if components.count < 2 {
      _log(String(describing: Self.self) + ": Incomplete message: c\(commandSuffix)", .warning, #function,  #file,  #line)
      return
    }
    let msgText = components[1]
    
    // log it
    _log(String(describing: Self.self) + ": \(msgText)", flexErrorLevel(errorCode: components[0]), #function, #file, #line)
  ...
  private func parseReply(_ replySuffix: String) {
    ...
    if components.count < 2 {
      _log(String(describing: Self.self) + ": Incomplete reply: r\(replySuffix)", .warning, #function, #file, #line)
      return
    }
    ...
    if let replyTuple = replyHandlers[ components[0].uValue ] {
      ...
      if components[1] != Api.kNoError {
        _log(String(describing: Self.self) + ": Unhandled non-zero reply: c\(components[0]), r\(replySuffix), \(flexErrorString(errorCode: components[1]))", .warning, #function, #file, #line)
      }
    }
  }
  /// Parse a Status
  ...
  private func parseStatus(_ commandSuffix: String) {
    ...
    guard components.count > 1 else {
      _log(String(describing: Self.self) + ": Incomplete status: c\(commandSuffix)", .warning, #function, #file, #line)
      return
    }
    ...
    guard let token = StatusToken(rawValue: msgType)  else {
      ...
      _log(String(describing: Self.self) + ": Unknown Status token: \(msgType)", .warning, #function, #file, #line)
      return
    }
    ...
    case .file:           _log(String(describing: Self.self) + ": Unprocessed \(msgType): \(remainder)", .warning, #function, #file, #line)
    ...
    case .mixer:          _log(String(describing: Self.self) + ": Unprocessed \(msgType): \(remainder)", .warning, #function, #file, #line)
    ...
    case .turf:           _log(String(describing: Self.self) + ": Unprocessed \(msgType): \(remainder)", .warning, #function, #file, #line)
      ...
      if version.isNewApi {
        ...
        if inUse {
          ...
          if properties[2].key == "forced" {
            ...
            // NO, Disconnected
            _log(String(describing: Self.self) + ": Disconnect, forced = \(properties[2].value)", .info, #function, #file, #line)
          }
        }
      }
    }
  ...
  private func parseDisplay(_ radio: Radio, _ keyValues: KeyValuesArray, _ inUse: Bool = true) {
    ...
    default:            _log(String(describing: Self.self) + ": Unknown Display type: \(keyValues[0].key)", .warning, #function, #file, #line)
    }
  }
      ...
      if radio.version.isNewApi && remainder.contains(Api.kRemoved) {
        ...
        if radio.version.isNewApi {
          ...
          guard let token = StreamTypeNew(rawValue: properties[1].value) else {
            ...
            _log(String(describing: Self.self) + ": Unknown Stream type: \(properties[1].value)", .warning, #function, #file, #line)
            return
          }
          switch token {
          ...
          guard let token = StreamTypeOld(rawValue: properties[1].key) else {
            // log it and ignore the Key
            _log(String(describing: Self.self) + ": Unknown Stream type: \(properties[1].key)", .warning, #function, #file, #line)
            return
          }
          switch token {
  ...
  private func parseInfoReply(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = InfoToken(rawValue: property.key) else {
        // log it and ignore the Key
        _log(String(describing: Self.self) + ": Unknown Info token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
      // Known keys, in alphabetical order
      switch token {
  ...
  private func parseVersionReply(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = VersionToken(rawValue: property.key) else {
        // log it and ignore the Key
        _log(String(describing: Self.self) + ": Unknown Version token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
  ...
  private func parseFilterProperties(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = RadioFilterSharpness(rawValue: property.key.lowercased())  else {
        // log it and ignore the Key
        _log(String(describing: Self.self) + ": Unknown Filter token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
  ...
  private func parseStaticNetProperties(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = RadioStaticNet(rawValue: property.key)  else {
        // log it and ignore the Key
        _log(String(describing: Self.self) + ": Unknown Static token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
  ...
  private func parseOscillatorProperties(_ properties: KeyValuesArray) {
    ...
    for property in properties {
      ...
      guard let token = RadioOscillator(rawValue: property.key)  else {
        // log it and ignore the Key
        _log(String(describing: Self.self) + ": Unknown Oscillator token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
        continue
      }
      // Known tokens, in alphabetical order
      switch token {
    ...
    if let category = RadioTokenCategory(rawValue: properties[0].key) {
      ...
      for property in properties {
        ...
        guard let token = RadioToken(rawValue: property.key)  else {
          // log it and ignore the Key
          _log(String(describing: Self.self) + ": Unknown Radio token: \(property.key) = \(property.value)", .warning, #function, #file, #line)
          continue
        }
        // Known tokens, in alphabetical order
        switch token {
  ...
  public func receivedMessage(_ msg: String) {
    ...
    default:    // Unknown Type
      _log(String(describing: Self.self) + ": Unexpected message: \(msg)", .warning, #function, #file, #line)
    }
  }
  ...
  public func defaultReplyHandler(_ command: String, sequenceNumber: SequenceNumber, responseValue: String, reply: String) {
    ...
    guard responseValue == Api.kNoError else {
      ...
      if !command.hasPrefix("client program ") {
        ...
        _log(String(describing: Self.self) + ": c\(sequenceNumber), \(command), non-zero reply \(responseValue), \(flexErrorString(errorCode: responseValue))", errorLevel, #function, #file, #line)
  ...
  public func vitaParser(_ vitaPacket: Vita) {
      ...
      if version.isNewApi {
        if let opus = remoteRxAudioStreams[vitaPacket.streamId] {
          ...
          if opus.isStreaming == false {
            ...
            _log(String(describing: Self.self) + ": Remote Rx Opus Stream started: Id = \(vitaPacket.streamId.hex)", .info, #function, #file, #line)
          }
        ...
        if let opus = opusAudioStreams[vitaPacket.streamId] {
          ...
          if opus.isStreaming == false {
            opus.isStreaming = true
            // log the start of the stream
            _log(String(describing: Self.self) + ": Opus Stream started: Id = \(vitaPacket.streamId.hex)", .info, #function, #file, #line)
          }
          opus.vitaProcessor( vitaPacket )
        }
      }
    ...
    case .panadapter:
      ...
      if let panadapter = panadapters[vitaPacket.streamId] {
        ...
        if panadapter.isStreaming == false {
          ...
          // log the start of the stream
          _log(String(describing: Self.self) + ": Panadapter Stream started: Id = \(vitaPacket.streamId.hex)", .info, #function, #file, #line)
        }
    ...
    case .waterfall:
      ...
      if let waterfall = waterfalls[vitaPacket.streamId] {
        ...
        if waterfall.isStreaming == false {
          ...
          // log the start of the stream
          _log(String(describing: Self.self) + ": Waterfall Stream started: Id = \(vitaPacket.streamId.hex)", .info, #function, #file, #line)
        }
    ...
    default:
      // log the error
      _log(String(describing: Self.self) + ": UDP Stream error, no object: \(vitaPacket.classCode.description()) Stream Id = \(vitaPacket.streamId.hex)", .error, #function, #file, #line)
    }
  }
